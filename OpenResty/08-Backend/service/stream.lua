local function msgpack_unit_helper(c)
    local tags = {
        [0xcc] = 1,
        [0xcd] = 2,
        [0xce] = 4
...
local c, err = tcpsock:receive(1)
local remains = msgpack_unit_helper(c)

local len
if remains == 0 then
    len = mp.unpack(c)
else
    local data = tcpsock:receive(remains)
    len = mp.unpack(c .. data)
end

local data, err = tcpsock:receive(len)
local msg = mp.unpack(data)
msg = string.rep(msg.str, msg.num)

local body = mp.pack(msg)
local header = mp.pack(#body)

local bytes = tcpsock:send(header .. body)
ngx.log(ngx.ERR, "send bytes: ", bytes)

-- tcpsock:shutdown("send")
