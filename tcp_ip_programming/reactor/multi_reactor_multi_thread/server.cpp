...
const int REACTOR_THREAD_COUNT = 4;
...
void reactorThread(int listenSocket) {
  ...
  while (true) {
    ...
    for (int i = 0; i < readyFdCount; ++i) {
      ...
      if (fd == listenSocket) {
        ...
        // 将连接套接字分配给一个工作线程处理
        int threadIndex = i % WORKER_THREAD_COUNT;
        mtx.lock();
...
int main() {
  ...
  // 创建多个 Reactor 线程
  std::vector<std::thread> reactorThreads;
  for (int i = 0; i < REACTOR_THREAD_COUNT; ++i) {
    reactorThreads.emplace_back(reactorThread, listenSocket);
  }

  // 等待 Reactor 线程退出
  for (auto& thread : reactorThreads) {
    thread.join();
  }
